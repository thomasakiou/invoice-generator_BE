...
def generate_thermal_receipt(buffer, receipt_data, logo_path=None, signature_path=None):
    ...
    if valid_items:
        for item in valid_items:
            ...
            story.append(Paragraph(f"{qty_display} x {currency_symbol} {format_number(item.unit_price)} = {currency_symbol} {format_number(item.quantity * item.unit_price)}", thermal_style))
    else:
